...
public struct JSON: Equatable {
	...
	public func wrapped(withMapKey key: String, asArray: Bool = false) -> JSON {
		if asArray {
			let values = values_from_scalar_or_array.map { $0.unwrap() }
...
extension JSON { // Helper methods
	...
	var as_array: JSON {
		if is_array {
			...
			return JSON.wrap(values_from_scalar_or_array.map { $0.unwrap() })
		}
	}
	...
	mutating func append(_ value: JSON) {
		guard case var .array(a) = self.value else {
			return
		}
		a.append(value)
		_wrappedValue = a
	...
	mutating func append<S>(contentsOf newElements: S) where JSON == S.Element, S: Sequence {
		guard case var .array(a) = value else {
			return
		}
	...
	func _is_map(with key: String) -> Bool {
		guard case let .map(dd) = value else {
			return false
		}
		if let _ = dd[key] {
			return true
